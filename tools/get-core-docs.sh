...
do
  # FIXME: use reset --hard instead of those 2 lines ?
	...
	then
		...
		$root/../php-documentation-generator/bin/pdg guides $root/content/$version/guides
		$root/../php-documentation-generator/bin/pdg references $root/core.temp/src $root/content/$version/references/ --base-url /docs/$version/references
	fi
done
